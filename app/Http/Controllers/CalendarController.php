            ...
            for ($i = 0; $i < 7; $i++) {
                ...
                if ( $dateGreaterThanToday && ($property_price->available($date)=='Not available') && ($property_price->type($date)=='calendar') && (($property_price->color($date))!=null)) {
                    $out .= '<div class="col-md-02">
                                <div class="calender_box date-package-modal"  style="background-color:'.$property_price->color($date).' !important " id="'.$date.'" data-day="'.$day.'" data-month="'.$month.'" data-year="'.$year.'" data-price="'.$property_price->original_price($date).'"data-status="'.$property_price->available($date).'"data-minday="'.$property_price->min_day($date).'">
                    ...
                    if (! $dateGreaterThanToday) {
                        $class = 'dt-not-available';
                        $out .= '<div class="col-md-02">
                                    <div class="calender_box date-package-modal '.$class.'" id="'.$date.'" data-day="'.$day.'" data-month="'.$month.'" data-year="'.$year.'" data-price="'.$property_price->original_price($date).'"data-status="'.$property_price->available($date).'"data-minday="'.$property_price->min_day($date).'">
                                        <div class="wkText final_day">'.$finalDay.' '.$today.'</div>
                                        <div class="dTfont wkText">'.$property_price->currency->org_symbol.$property_price->original_price($date).'</div>
                                    </div>
                                </div>';
