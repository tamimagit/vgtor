...
class PayoutController extends Controller
{
    ...
    public function setting(Request $request)
    {
        ...
        if (!$request->isMethod('post')) {
            ...
            if ($request->payout_type == 1) { // paypal
                $rules = array(
            ...
            } else if ($request->payout_type == 4) { // bank
                $rules = array(
            ...
            } else if ($request->payout_type == 5) { // mobile
                $payoutSetting = new PayoutSetting;
                $payoutSetting->user_id = Auth::user()->id;
                $payoutSetting->type = $request->payout_type;
                $payoutSetting->email = Auth::user()->email;
                $payoutSetting->country = $request->country;
                $payoutSetting->mobile_banking_id = $request->mobile_banking;
    ...
    public function edit(Request $request)
    {
        if (!$request->isMethod('post')) {
            ...
            if ($request->payout_type == 1) { // paypal
                ...
                if ($validator->fails()) {
                    $this->helper->one_time_message('error', "Update Failed");
                    return back()->withErrors($validator)->withInput();
                } else {
                    $payoutSetting = PayoutSetting::find($request->id);
                    $payoutSetting->user_id = Auth::user()->id;
    ...
    public function success(Request $request)
    {
        ...
        if ($walletMoney->balance_indefault < $request->amount) {
    ...
    public function getMobileBanking(Request $request)
    {
        return MobileBanking::where('country_id', $request->country_id)->get();
    }
}
