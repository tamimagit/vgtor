...
@section('main')
    <div class="margin-top-85">
                    ...
                    <div class="main-panel">
                        ...
                        <div class="row mt-5">
                            <div class="col-md-12 p-0">
                                <form class="form-horizontal pl-0 pr-0 pl-md-4 pr-md-4" enctype='multipart/form-data'
                                      action="{{ url('users/payout-list') }}" method="GET" id='filter_form'
                                    ...
                                    <div class="row justify-content-between">
                                        <div class="d-flex rounded-3 pt-3 pb-3  border">
                                            <div class="pl-3 pr-3">
                                                <button type="button"
                                                        class="form-control pick_date pick_date-width pick-btn"
                                                        id="daterange-btn">
                                            ...
                                            <div class="text-right pl-3 pr-3">
                                                <button type="submit" name="btn"
                                                        class="btn vbtn-outline-success text-14 font-weight-700 pl-4 pr-4 pt-3 pb-3 mr-2">{{trans('messages.filter.filter')}}</button>
                                            </div>
                                        </div>
                                        ...
                                        <div class="mt-4 mt-sm-0">
                                            @if($payouts->count() > 0)
                                                <button type="button"
                                                        class="btn vbtn-outline-success text-14 font-weight-700 pl-4 pr-4 pt-3 pb-3"
                                            ...
                                            @else
                                                <button type="button"
                                                        class="btn vbtn-outline-success text-14 font-weight-700 pl-4 pr-4 pt-3 pb-3 no-payout">
                                                    {{trans('messages.account_preference.payout_request')}}
                                                    <i class="fa fa-arrow-up"></i>
                                                </button>
                                            @endif
        ...
        <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" data-keyboard="false"
             data-backdrop="static" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header p-4">
                        <h2 class="modal-title"
                            id="exampleModalLabel">{{trans('messages.account_preference.payout_request')}}</h2>
                    ...
                    <div class="modal-body p-4">
                        <form action="{{url('users/payout/success')}}" id="add_payout_request" method="post"
                            ...
                            <div class="row" id="paymentDiv">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <label for="exampleInputPassword1"
                                               class="control-label">{{trans('messages.utility.payment_method')}}</label>
                                        <select class="form-control text-14" name="payment_method_id"
                                ...
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <label for="exampleInputPassword1"
                                               class="control-label">{{trans('messages.listing_price.currency')}}</label>
                                        <select class="form-control text-14" name="currency_id" id="currency_id">
                                            <option
                                                value="{{$default_currency->id}}">{{$default_currency->code}}</option>
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <label for="exampleInputPassword1"
                                               class="control-label">{{trans('messages.account_transaction.amount')}}
                                        ...
                                        <input type="text" class="form-control text-14" name="amount" id="amount"
                                               value="{{old('amount')}}">
                                        <span class="text-danger d-none" id="amount_high">Don't have sufficient balance !</span>

                                        @if ($errors->has('amount')) <p
                                            class="error-tag">{{ $errors->first('amount') }}</p>
                                        @endif
                                    </div>
                                </div>

                                <input type="text" name="balance" id="balance"
                                       value="{{ convert_currency($walletBalance->currency->code, $default_currency->code, $walletBalance->balance) }}"
                                       hidden="">

                                <div class="col-md-12 text-right mt-4">
                                    <button type="button" class="btn btn-outline-danger text-14 mt-2 pl-4 pr-4 mr-2 "
                                            id="close" data-dismiss="modal">{{trans('messages.utility.close')}}</button>
                                    <button type="button" class="btn vbtn-outline-success text-14 mt-2 pl-4 pr-4 ml-2"
                                            disabled id="next_btn">  {{trans('messages.utility.next')}} </button>
                                </div>
                            </div>
                ...
                $(document).ready(function () {
                    ...
                    $('#next_btn').on('click', function () {
                        if ($('#next_btn').prop('disabled')) {
                            ...
                            } else if (item.type == 5) { // mobile
                                $('#confirmDiv').html('<div class="row">'
                                    + '<div class="col-md-12">'
                                    + '<div class="form-group">'
                                    + '<label><strong class="font-weight-700">Mobile Banking :</strong> ' + item.mobile_banking_number + '</label>'
                                    + '</div>'
                                    + '</div'
                                    + '<div class="col-md-12">'
                                    + '<div class="form-group">'
