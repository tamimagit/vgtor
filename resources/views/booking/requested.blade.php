            ...
            <div class="col-md-8 mb-5 mt-3 main-panel p-5 border rounded">
                <div class="col-lg-12 p-0">
                    @if($booking_details->status == 'Pending')
                        @if($booking_details->payment_method_id == 4 OR $booking_details->payment_method_id == 5)
                            <h2>{{trans('messages.booking_request.get_ready')}} {{ $booking_details->properties->property_address->city }}
                                !</h2>
                            <p>{{trans('messages.booking_request.bank_paid')}}</p>
                        @else
                            <h2 class="font-weight-700">{{ trans('messages.booking_request.request_has_sent') }}</h2>
                            <p>
                                {{ trans('messages.booking_request.not_a_confirmed_booking') }} {{ trans('messages.booking_request.hear_back_within_24') }} {{ trans('messages.booking_request.not_be_charged') }} {{$booking_details->properties->users->first_name}} {{ trans('messages.booking_request.accommodate_stay') }}
                                .
                            </p>
                        @endif
                    @endif
                    ...
                    @if($booking_details->status == 'Accepted')
                        <h2>{{trans('messages.booking_request.get_ready')}} {{ $booking_details->properties->property_address->city }}
                            !</h2>
                        <p>{{trans('messages.booking_request.confirmed_booking')}} {{$booking_details->properties->users->first_name}}
                            . {{trans('messages.booking_request.emailed_itinerary')}} {{$booking_details->properties->users->email}}
                            .</p>
                    @endif
                </div>
            </div>
            ...
            <div class="col-md-4">
                <div class="card mt-3 mb-5 p-3">
                    <a href="{{ url('/') }}/properties/{{ $booking_details->properties->slug}}">
                        <img class="card-img-top p-2 rounded" src="{{$booking_details->properties->cover_photo}}"
                             alt="{{$booking_details->properties->name}}" height="180px">
                    </a>
                    ...
                    <div class="card-body p-4">
                        ...
                        <p class="text-14 mt-2 text-muted mb-0">
                            ...
                            {{$booking_details->properties->property_address->address_line_1}}
                            , {{ $booking_details->properties->property_address->state }}
                            , {{ $booking_details->properties->property_address->country_name }}
                        </p>
                        <div class="border p-4 mt-3 text-center rounded-3">
                            <p class="text-16 mb-0">
                                <strong
                                    class="font-weight-700 secondary-text-color">{{ $booking_details->properties->property_type_name }}</strong>
                                {{trans('messages.payment.for')}}
                                <strong
                                    class="font-weight-700 secondary-text-color">{{ $booking_details->guest }} {{trans('messages.payment.guest')}}</strong>
                            </p>
                            <div class="text-16">
                                <strong>{{ date('D, M d, Y', strtotime($booking_details->startdate_dmy)) }}</strong> to
                                <strong>{{ date('D, M d, Y', strtotime($booking_details->enddate_dmy)) }}</strong></div>
                        </div>
                            ...
                            @if($booking_details->host_fee)
                                <div class="d-flex justify-content-between text-16">
                                    <div>
                                        ...
                                        <i class="icon icon-question icon-question-sign" data-behavior="tooltip"
                                           rel="tooltip"
                                           aria-label="Vrent charges a fee to cover the cost (banking fees) of processing payment from the traveler."></i>
